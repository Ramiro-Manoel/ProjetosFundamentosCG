...
int setupShader();
int setupGeometry();
int setupSpiral(float middleX, float middleY, float maxRadius, int num_segments);
...
//#endif
	...
	GLuint VAO = setupSpiral(0.0, 0.0, 5, numberOfVertices);
...
int setupSpiral(float middleX, float middleY, float maxRadius, int num_segments)
{
    GLfloat vertices[num_segments * 3 + 3]; // Num vertices + centro (primeiro ponto) e o final
	float radius = 0.0;
	float celcius = 0.0;
	
	for (int i = 0; i < num_segments; i++)
	{
		float angle = 2.0f * 3.1415926f * celcius / num_segments;
